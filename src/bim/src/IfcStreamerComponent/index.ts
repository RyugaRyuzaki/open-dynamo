 ...
 */
export class IfcStreamerComponent
  ...
  implements OBC.Disposable
{
   ...
   */
  static readonly uuid = "5e9efcc0-cf05-4ece-84f1-87b766633c9d" as const;
  ...
  private async getGeometryFile(geometryFile: string) {
    ...
    for (const modelID in seen) {
      ...
      for (const [file] of sortedFiles) {
        ...
        if (!this._ramCache.has(file)) {
          const bytes = await this.getGeometryFile(file);
          if (bytes) {
